...
function theme1_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'theme1' ),
...
//		$categories_list = get_the_category_list( esc_html__( ', ', 'theme1' ) );
//		if ( $categories_list && theme1_categorized_blog() ) {
//			printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'theme1' ) . '</span>', $categories_list ); // WPCS: XSS OK.
//		}
//
//		/* translators: used between list items, there is a space after the comma */
//		$tags_list = get_the_tag_list( '', esc_html__( ', ', 'theme1' ) );
//		if ( $tags_list ) {
//			printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'theme1' ) . '</span>', $tags_list ); // WPCS: XSS OK.
//		}
//	}
	...
	if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( sprintf( wp_kses( __( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'theme1' ), array( 'span' => array( 'class' => array() ) ) ), get_the_title() ) );
		echo '</span>';
	}
